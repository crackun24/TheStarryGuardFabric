...
public class Config {//配置文件类
    static final String CONFIG_FILE_NAME = "TheStarryGuard.properties";//配置文件的名称
    ...
    static public Config LoadConfig(String file_path) throws IOException//加载配置文件
    {
        ...
        File file = new File(file_path + "\\" + CONFIG_FILE_NAME);//打开文件
        if (!file.exists())//判断文件是否存在
        {
            CreateDefaultConfigFile(file);//如果文件不存在,则直接创建一个新的文件,并且写入默认的配置文件信息
        }

        Reader reader = new InputStreamReader(new FileInputStream(file), StandardCharsets.UTF_8);//以UTF-8的字符集读取配置文件数据
