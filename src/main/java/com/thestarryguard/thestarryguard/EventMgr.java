...
import com.thestarryguard.thestarryguard.Events.*;
...
public class EventMgr {
    ...
    private void HookBlockBreakEvent() {
        PlayerBlockBreakEvents.BEFORE.register((world, player, pos, state, blockEntity) -> {
            ...
            if (this.dataQuery.IsPlayerHookPointQuery(player_name)) {
                ...
                Action action = new Action(Action.BLOCK_BREAK_ACTION, new Player(player.getName().getString(),
                        player.getUuidAsString()),
                        block_id, pos.getX(), pos.getY(), pos.getZ(), dimension_name, null);
    ...
    private void HookPlayerFireBlockEvent()//判断是否注册了玩家点燃方块的事件
    {
        PlayerFireBlockEvent.EVENT.register((world, player, blockState, pos) -> {

            String dimension_name = world.getRegistryKey().getValue().toUnderscoreSeparatedString();//获取世界的名字
            String block_id = Registries.BLOCK.getId(blockState.getBlock()).toString();

            Action action = new Action(Action.FIRE_BLOCK, new Player(player.getName().getString(),
                    player.getUuidAsString()),
                    block_id, pos.getX(), pos.getY(), pos.getZ(), dimension_name, null);
            this.dataStorage.InsertAction(action);
    ...
    private void HookEntityAttackEvent() {
        AttackEntityCallback.EVENT.register((player1, world1, hand1, entity, hitResult) -> {
            ...
            Action action = new Action(Action.ATTACK_ACTION, new Player(player1.getName().getString(), player1.getUuidAsString()),
    ...
    private void HookKillEntityEvent()//注册杀死实体的事件
    {
        PlayerKillEntityEvent.EVENT.register((world, killer, other) -> {
            ...
            Action action = new Action(Action.KILL_ENTITY_ACTION, new Player(killer.getName().getString(), killer.getUuidAsString()),
    ...
    private void HookPlayerUseTntEvent()//注册玩家使用TNT的事件
    {
        PlayerLitTntEvent.EVENT.register((state, world, pos, player, hand, hit) -> {
            String dimension_name = world.getRegistryKey().getValue().toUnderscoreSeparatedString();//获取世界的名字
            String block_id = Registries.BLOCK.getId(world.getBlockState(pos).getBlock()).toString();

            Action action = new Action(Action.TNT_USE, new Player(player.getName().getString(),
                    player.getUuidAsString()),
                    block_id, pos.getX(), pos.getY(), pos.getZ(), dimension_name, null);
            this.dataStorage.InsertAction(action);
            return ActionResult.PASS;
    ...
    private void HookKillPlayerEvent()//注册玩家杀死玩家事件
    {
        PlayerKillPlayerEvent.EVENT.register((world, killer, player) -> {
            ...
            Action action = new Action(Action.KILL_PLAYER_ACTION,
    ...
    public void HookEvent()//注册所有在配置文件中启用的事件
    {
        ...
        if (Boolean.parseBoolean(config.GetValue("hook_player_fire_block_event"))) {
            HookPlayerFireBlockEvent();
        }
