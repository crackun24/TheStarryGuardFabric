...
public class Action {//玩家的行为类
    ...
    public enum ActionType {BLOCK_BREAK, BLOCK_PLACE, ATTACK}//玩家行为的枚举

    public enum TargetType {ENTITY, PLAYER, BLOCK}//目标的类型

    public enum Dimension {THE_END, OVER_WORLD, THE_NETHER};//维度的枚举
    ...
    public final Dimension dimension;//玩家触发这个事件的维度
    public final TargetType targetType;//事件的类型
    public final String eventData;//事件的数据

    public Action(ActionType type, TargetType targetType, String playerUUID, String targetName, int x, int y, int z, Dimension dimension, String eventData) {
